    ...
    <OptionEntry>
        ...
        <div class="paths" slot="value">
            {#each newPath as path, i}
                ...
                {/if}
                <input id="new-path" type="text" on:keyup={stripEmptyPath} size={Math.max(1, path.length)} bind:value={path}>
            {/each}
            <IconButton on:click={() => {newPath = [...newPath, ""]}}><PlusIcon size={16} /></IconButton>
        </div>
    </OptionEntry>
