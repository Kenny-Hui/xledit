<script lang="ts">
    ...
    function createNewGroup(event) {
        ...
        if ($selectedFile.isSource) {
            fileAffected = $projects.files.filter(
                (e) => e.sourceLanguage == $selectedFile.sourceLanguage,
            );
        } else {
...
<OptionEntry>
    <div slot="key">
        {#if $selectedFile.isSource}
            <p class="note">
                You are creating a group on all files translating from {$selectedFile.sourceLanguage}
            </p>
        {:else}
            <p class="note">
                You are creating a group in this file. ({$selectedFile.targetLanguage})<br
                />Please edit the source file ({$selectedFile.sourceLanguage})
                if you wish to apply to all files.
            </p>
        {/if}
    </div>
</OptionEntry>
