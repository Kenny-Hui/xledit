<script lang="ts">
    ...
    import { getSortPoint } from '../../../../utils/util';
            ...
            for(let file of $projects.files) {
                forEach(file.rootGroup, (data: Unit | Group) => {
                    if(data instanceof Unit) {
                        ...
                        // No better than unfinished translation, as it's literally the source but copied
                        ...
                        if(matchPercent > SUGGESTION_MATCH_PERCENTAGE) {
                            matchedUnit.push({
                                ...
                                targetLang: file.targetLanguage,
                                uuid: file.uuid,
                                file: file.filename
                            });
                            sortSuggestions();
    ...
    function sortSuggestions() {
        ...
        matchedUnit.sort((a, b) => {
            ...
            let aSameLangGroup = a.targetLang.split("-")[0] == targetLanguage.split("-")[0] ? -1 : 0;
            let bSameLangGroup = b.targetLang.split("-")[0] == targetLanguage.split("-")[0] ? 1 : 0;
            ...
            point += (a.targetLang.split("-")[0] == b.targetLang.split("-")[0]) ? 1 : 0; // Match primary subtag
...
<div class="suggestion">
    ...
    {:else}
        {#each matchedUnit as unit}
            <SuggestionEntry on:applyTargetText {unit} {selectedUnit} />
        {/each}
    {/if}
</div>
