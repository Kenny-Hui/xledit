<script lang="ts">
	...
	function meetCriteria(data: Unit | Group, query: SearchQuery): boolean {
		if (data instanceof Group) {
			...
			return (
				data.id.toLowerCase().includes(queryLowerCase) ||
				data.source.text.toLocaleLowerCase().includes(queryLowerCase)
			);
		}
	}

	function meetFilter(unit: Unit, query: SearchQuery) {
		if (!query.showNumericOnly && /^\d+$/.test(unit.source.text))
			return false;
	...
	function add() {
		openDialog(
			new DialogProperty(CreateDialog, { path: group.path }, () => {
				group = group;
				expanded = true;
			}),
		);
	...
	$: group.groups,
		group.units,
		(items =
			group.groups.filter((e) => meetCriteria(e, searchQuery)).length +
			group.units.filter((e) => meetCriteria(e, searchQuery)).length);

	$: if (
		$selectedUnit &&
		$selectedUnit.getFullPathStr().startsWith(group.path.join("/"))
	)
		expanded = true;
