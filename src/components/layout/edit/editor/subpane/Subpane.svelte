<script lang="ts">
        ...
        for (let file of $projects.files) {
            forEach(file.rootGroup, (data: Unit | Group) => {
                if (data instanceof Unit) {
                    ...
                    if (
                        data.target.text == "" ||
                        data.source.text == data.target.text
                    )
                        return;
                    // Same as our result
                    if (
                        data.source.text == $selectedUnit.source.text &&
                        data.target.text == $selectedUnit.target.text
                    )
                        return;

                    let matchPercent = stringSimilarity(
                        $selectedUnit.source.text,
                        data.source.text,
                    );
                    if (matchPercent > SUGGESTION_MATCH_PERCENTAGE) {
                        if (
                            matchedUnit.filter((e) => e.unit == data).length ==
                            0
                        ) {
                            // Slight convenient hack to prevent duplicate, since this is async recursion
                            matchedUnit.push({
                                unit: data,
    ...
    function sortSuggestions(arr: any[]) {
        let targetLanguage =
            $selectedFile.targetLanguage ?? $selectedFile.sourceLanguage;
        ...
        arr.sort((a, b) => {
            ...
            let aSamePath =
                a.unit instanceof Unit
                    ? a.unit.getFullPathStr() == $selectedUnit.getFullPathStr()
                        ? 1
                        : 0
                    : 0;
            let bSamePath =
                b.unit instanceof Unit
                    ? b.unit.getFullPathStr() == $selectedUnit.getFullPathStr()
                        ? 1
                        : 0
                    : 0;
            ...
            let aSameLangGroup =
                a.lang.targetLanguage == null
                    ? 0
                    : a.lang.targetLanguage.split("-")[0] ==
                        targetLanguage.split("-")[0]
                      ? 1
                      : 0;
            let bSameLangGroup =
                b.lang.targetLanguage == null
                    ? 0
                    : b.lang.targetLanguage.split("-")[0] ==
                        targetLanguage.split("-")[0]
                      ? 1
                      : 0;
    ...
    $: {
        if ($selectedFile != null && $selectedUnit != null) {
            findSuggestions();
            tabs[1].count =
                $selectedUnit.notes.length + $selectedUnit.contextGroups.length;
        } else {
            tabs[0].count = 0;
            tabs[1].count = 0;
        }
