...
export function forEachBlocking(
  group: Group,
  callback: (data: Unit | Group) => void,
) {
  for (let unit of group.units) {
    callback(unit);
  }
...
export function createGroup(
  ...
  if (str.length <= 1) {
    // HACK: Inconsistent behaviour with unit (Where unit's path does not include the ID, and group does)
