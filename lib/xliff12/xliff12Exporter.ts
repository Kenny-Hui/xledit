...
function exportTranslationFile(
  ...
  if (data.targetLanguage != null)
    elem.setAttribute("target-language", data.targetLanguage);
  if (!elem.hasAttribute("original"))
    elem.setAttribute("original", data.filename);
...
function exportNote(
  ...
  if (data.priority != 0)
    elem.setAttribute("priority", data.priority.toString());
  if (data.annotates != "general")
    elem.setAttribute("annotates", data.annotates);
  elem.textContent = escapeXml(data.content);

  return elem;
}

function exportContextGroup(
  data: ContextGroup,
  xml: XMLDocument,
  xliffElement: Element,
): Element {
  let elem = createXmlElement(
    "context-group",
    xml,
    xliffElement,
    data.metadata,
...
function exportHeader(
  ...
  if (data.skl != null) {
    elem.appendChild(
      exportInternalExternalFile("skl", data.skl, xml, xliffElement),
    );
  }

  for (let glossary of data.glossaries) {
    elem.appendChild(
      exportInternalExternalFile("glossary", glossary, xml, xliffElement),
    );
  }

  for (let reference of data.references) {
    elem.appendChild(
      exportInternalExternalFile("reference", reference, xml, xliffElement),
    );
  }
...
function exportInternalExternalFile(
  name: string,
  data: InternalExternalFile,
  xml: XMLDocument,
  xliffElement: Element,
): Element {
