...
export function parseXliff12(filename: string, data: string): TranslationFile[] {
    ...
    if(xml.querySelector('parsererror')) { //Error while parsing XML
...
function parseFile(elem: Element, filename: string) {
    let header = elem.querySelector("header") != null ? parseHeader(elem.querySelector("header")) : null;
...
function parseContextGroup(elem: Element) {
    ...
    for(let contextElem of elem.getElementsByTagName("context")) {
        contexts.push(new Context(contextElem.textContent, contextElem.getAttribute("context-type") as ContextType, contextElem.attributes));
    }
